...
# 2051105 edit junohpark for BOSS CARD
for CONTROLLER in $($OMSABIN storage controller | grep ^ID | awk '{print $3}')
do
...
case "$3" in
	...
	;;
    size)
	echo "$($OMSABIN storage pdisk controller=$CONTROLLER pdisk=$PDISK | grep ^Capacity | awk '{print $3 $4}')"
	;;
    media)
	echo "$($OMSABIN storage pdisk controller=$CONTROLLER pdisk=$PDISK | grep ^Media | awk '{print $3}')"
	;;
    type)
	echo "$($OMSABIN storage pdisk controller=$CONTROLLER pdisk=$PDISK | grep ^Bus | awk '{print $4}')"
	;;
    product)
	echo "$($OMSABIN storage pdisk controller=$CONTROLLER pdisk=$PDISK | grep ^Product | awk '{print $4}')"
	;;
	pfailure)
...
STATUS="$($OMSABIN storage controller | grep "^Name" | cut -d':' -f2 | sed 's/^ //')"
echo "$STATUS"
...
STATUS="$($OMSABIN licenses| grep ^Description | grep iDRAC  | cut -d':' -f2)"
echo "$STATUS"
}
